...
const ForumThread = (props) => {
  ...
  const respondingToID = postStore((state) => state.respondingToID);
  const setRespondingToID = postStore((state) => state.setRespondingToID);
  ...
  const ResponseForm = () => {
    ...
    const handleClick = (e) => {
      e.preventDefault();
      responseObj['response_to_id'] = respondingToID;
    ...
    <>
      <div className="forumThread">
        {responsesData.map((response) =>
          <ForumMessage key={response.id} response={response} handleReplyClick={handleReplyClick} />,
        )}
      </div>
